    ...
    if (!this.__execs__.autoResize) {
      ...
      this.__execs__.autoResize = function () {
        if (allowResize && that.__execs__.canvas) {
          var rect = that.__execs__.container.node().getBoundingClientRect();
          if (!lastWidth || lastWidth !== rect.width) {
            var transition$$1 = that.transition();
            that.width(rect.width).transition({ duration: 0, delay: 0 }).render(true, that.autoResizeSkip()).transition(transition$$1);
...
function gen(type, call, option) {
  return { type: type, call: call, allow: option.allow, isPre: option.isPre };
}
/**
...
function process(type, call) {
  ...
  if (call && typeof call === 'function') {
    ...
    return process.find(function (p) {
      return p.type === type;
    });
  }
}
...
function render() {
  ...
  this.process().forEach(function (p) {
    if (skip.indexOf(p.type) < 0) {
      if (p.isPre) process.pre.push(p);else process.post.push(p);
    }
  });
  ...
  if (node && node.size() > 0) {
    ...
    node.call(bindOn, dispatch$$1).on('mouseenter.default.legend', function (d) {
      if (legend && legend.mute && _this2.muteToLegend) _this2.muteToLegend(d);
    }).on('mouseleave.default.legend', function (d) {
      if (legend && legend.demute && _this2.demuteToLegend) _this2.demuteToLegend(d);
    });
  }
...
function _render$1(selection) {
  ...
  var title = area$$1.selectAll(className('title', true)).data([titleText]);
  title.exit().remove();
...
var titleSize = 13;
var defaultDomainColor = '#c0ccda';
var titleColor = '#485465';
var defaultGridColor = '#3d4145';
var zerocolor = '#aaa';
...
function _grid(selection, zoomed) {
  if (this.grid() && this.gridSize() > 0) {
    ...
    gridLineEnter.append('line').style('stroke', this.gridColor());
...
function _preStyle(selection) {
  ...
  for (var k in font) {
    if (k !== 'fill') selection.style(k, k === 'font-size' ? font[k] + 'px' : font[k]);
  }
}
...
function _style$2(selection) {
  ...
  selection.select('.domain').style('stroke', this.domainColor()).style('shape-rendering', 'crispEdges').style('stroke-width', '1px').style('stroke-opacity', this.showAxisLine() ? 1 : 0).style('visibility', this.showDomain() ? 'visible' : 'hidden'); //showDomain
...
function axisDefault(scale, axisSetting) {
  ...
  ['tickPadding', 'thickness', 'showTitle', 'showDomain', 'showTicks', 'font', 'domainColor', 'showAxisLine', 'gridColor', 'tickSize'].forEach(function (k) {
...
function _domain() {
  ...
  var diffArrow = this.diffArrow();
  var arrowOnMark = this.arrowOnMark();
  ...
  if (yDomain[0] * yDomain[1] < 0 && diffArrow) {
    if (Math.abs(yDomain[0] + yDomain[1]) > (Math.abs(yDomain[0]) + Math.abs(yDomain[1])) / 3) {
      var isPlus = yDomain[0] + yDomain[1] > 0 ? 1 : 0;
...
function _mark() {
  ...
  var data = this.data();
  var arrowOnMark = this.arrowOnMark();
  var downArrow = this.downArrowPath();
  var upArrow = this.upArrowPath();
  ...
  var __labelInit = function __labelInit(selection) {
    ...
    selection.each(function (d) {
      ...
      if (vertical) {
        ...
        if (stacked) selection.attr('y', d.y + d.h).attr('dy', '1em');else if (d.key !== "") {
          if (d.upward) selection.attr('y', d.y + d.h).attr('dy', '-.25em');else selection.attr('y', d.y).attr('dy', '1em');
        }
      } else {
  ...
  var __bar = function __bar(selection) {
    selection.transition(trans).attr('x', function (d) {
      ...
      if (d3.select(this).classed(className("diff-arrow"))) return "none";else return d.color;
    });
  };
  var __label = function __label(selection) {
    ...
    selection.each(function (d) {
      ...
      .style('fill', '#fff').style('visibility', label && (!diffColor || diffColor && selection.classed(className('diff'))) ? 'visible' : 'hidden');
      if (d.key === "") {
  ...
  if (diffArrow) {
    ...
    var arrowWidthFactor = this.arrowWidthFactor();
    bar.each(function (d, i, arr) {
      if (i === diffArrow["pos"]) {
        d.neighbor = _last;
        d3.select(this).append("image").attr("class", className("diff-arrow"));
        d3.select(this).append("text").attr("class", className("diff-arrow"));
        ...
        // console.log(d);
        d3.select(this).select("text" + className("bar", true)).each(function (e) {
          var i = d3.select(this).transition(trans);
          i.attr("y", d.y);
        });
        d3.select(this).select("rect" + className("bar", true)).transition(trans);
        d3.select(this).select("image" + className("diff-arrow", true)).attr("x", d.x).attr("y", d.y).attr("width", vertical ? d.w : 0).attr("height", vertical ? 0 : d.h).attr("xlink:href", diffArrow["value"] < 0 ? downArrow : upArrow).call(__bar).style("stroke", "#40bbfb");
        d3.select(this).select("text" + className("diff-arrow", true)).datum(d).attr("y", text_y).call(__labelInit, vertical).call(__label, vertical).style("fill", "#40bbfb");
      } else {}
      _last = d;
    });
  }
  ...
  if (arrowOnMark) {
    var _strokeWidth2 = 1;
    var _halfStrokeWidth = _strokeWidth2 / 2;
    ...
    bar.each(function (d, i, r) {
      ...
      diff.y = 30;
      d3.select(this).select("text" + className("markarrow", true)).each(function (e) {
        var selection = d3.select(this).transition(trans);
        selection.attr("y", e.y - 100);
      });
      d3.select(this).select("rect" + className("bar", true)).transition(trans);
      d3.select(this).select("image" + className("markarrow", true)).datum(diff).attr("x", vertical ? diff.x + _halfStrokeWidth : 0).attr("y", vertical ? d.upward ? d.y + d.h : d.y : diff.y + _halfStrokeWidth).attr("width", vertical ? d.w - _strokeWidth2 : 0).attr("height", vertical ? 0 : d.h - _strokeWidth2).attr("xlink:href", diff.value < 0 ? downArrow : upArrow).call(__bar).style("stroke", "#40bbfb");
      d3.select(this).select("text" + className("markarrow", true)).datum(diff).call(__labelInit, vertical).call(__label, vertical).style("fill", "#40bbfb");
    });
  }
...
var DimensionField = function (_Field) {
  ...
  createClass(DimensionField, [{
    ...
    value: function domain() {
      ...
      var multipleDomain = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;

      var munged = this.munged();
      var level = this.level();
      var order = this.order();
      var needMultipleDomain = sortByValue !== 'natural' && multipleDomain;
      var curLevel = 0;
      var duplicateDomain = [];
      function _fill(nodes, setDomain, curLevel) {
        if (curLevel === level) {
          var residual = setDomain.filter(function (d) {
      ...
      function _keys(nodes, curLevel) {
        if (curLevel === level) {
          var _domain2 = nodes.map(accessor ? accessor : function (d) {
            return { key: d.data.key, value: d.value };
          });
          if (needMultipleDomain) {
            duplicateDomain = duplicateDomain.concat(_domain2);
            return _domain(_domain2, false);
          } else {
            return _domain2;
          }
        } else {
          if (needMultipleDomain) {
            ...
            return d3.merge(nodes.map(function (d) {
      ...
      function _domain(domain) {
        var duplicate = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
      ...
      function _set(domain) {
        ...
        domain.forEach(function (d) {
          if (!newDomain.find(function (n) {
            return n.key === d.key;
          })) newDomain.push(d);
        });
        return newDomain;
      }
      var domain = _keys(munged, curLevel);
      if (needMultipleDomain) {
        var domainSet = _set(duplicateDomain).map(function (d) {
          return d.key;
        });
        curLevel = 0;
...
function translate$1(selection, innerSize, pos) {
...
function _unit() {
    ...
    var unit_text = canvas.select(".unit-text");
    unit_text.text(unit ? unit['text'] ? unit['text'] : unit : "").style("font-family", "Noto Sans KR").style("font-size", "22px").style("font-weight", "600").style("fill", "#C6CBCE").attr("text-anchor", "end").call(translate$1, innerSize, unit ? unit['pos'] ? unit['pos'] : [0, 0] : [0, 0]);
}

function translate$2(selection, innerSize, pos) {
...
function _title$1() {
    ...
    var title_text = canvas.select(".title_text");
    title_text.text(title ? title['text'] ? title['text'] : title : "").style("font-family", "Noto Sans KR").style("font-size", "40px").style("font-weight", "bold").style("fill", "#FFFFFF").attr("text-anchor", "start").call(translate$2, innerSize, title ? title['pos'] ? title['pos'] : [0, 0] : [0, 0]);
}
...
var Bar = function (_mix$with) {
  ...
  function Bar() {
    ...
    _this.process('munge', _munge, { isPre: true }).process('domain', _domain, { isPre: true }).process('range', _range, { isPre: true }).process('unit', _unit).process('title', _title$1).process('axis', _axis$1).process('region', _region).process('facet', _facet, { allow: function allow() {
        return this.isFacet();
  ...
  createClass(Bar, [{
    ...
    value: function isNestedAndSortByValue() {
...
function domainY(yField, munged) {
  ...
  var label = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : false;
  var arrowOnMark = arguments.length > 8 && arguments[8] !== undefined ? arguments[8] : false;
  ...
  if (arrowOnMark) {
    if (yDomain[0] === 0) yDomain[1] *= 1.25;else if (yDomain[1] === 0) yDomain[0] *= 1.25;else {
      yDomain[1] *= 1.5;
  ...
  if (label) {
    if (yDomain[0] === 0) yDomain[1] *= 1.1;else if (yDomain[1] === 0) yDomain[0] *= 1.1;else {
      if (yDomain[0] + yDomain[1] > 0) {
...
function _mark$2() {
    ...
    var diffWithArrow = this.diffWithArrow();
    var downArrow = this.downArrowPath();
    var upArrow = this.upArrowPath();
    var font = this.font();
    ...
    var __seriesInit = function __seriesInit(selection) {
        ...
        if (area$$1) {
            ...
            selection.each(function (d) {
                ...
                var values = target.map(function (d) {
                    return d.value;
                });
    ...
    var __series = function __series(selection) {
        ...
        if (area$$1) {
            ...
            if (areaGradient) {
                ...
                selection.attr('stroke', 'none').attr("fill", url);
                // console.log('c', c)
            } else if (area$$1) {
    ...
    var __pointInit = function __pointInit(selection) {
        selection.attr('r', (size.range[0] - size.range[0] / 4) * pointRatio).attr('stroke-width', size.range[0] / 4 * pointRatio)
        // .style('fill', '#fff')
        .attr('opacity', showPoint ? 1 : 0).style('cursor', 'pointer').attr('cx', function (d) {
            return d.x0 || d.x;
        }).attr('cy', function (d) {
    ...
    var __labelInit = function __labelInit(selection) {
        selection.each(function (d) {
            ...
            selection.attr('x', function (d) {
                ...
                return stream ? d.y : d3.max(scale.y.range());
            }).attr('stroke', 'none').style("fiil", "#ffffff").style('visibility', label ? 'visible' : 'hidden').text(d.text);
            that.styleFont(selection);
        });
    };
    var __label = function __label(selection) {
        selection.each(function (d) {
            ...
            selection.attr('text-anchor', d.anchor).style('pointer-events', multiTooltip ? 'none' : 'all').transition(trans)
            // .attr('y', d.y  + (size.range[0] / 2 * pointRatio + 1) * (d.upward ? 1 : -1))
            .attr('y', d.y + font["font-size"] * .75 / 2).attr('x', d.x)
            // .attr('dy', d.upward ? '.71em' : 0)
            .style('visibility', label ? 'visible' : 'hidden').text(d.text);
            that.styleFont(selection);
        });
    };
    ...
    var __appendArrow = function __appendArrow(selection) {
        ...
        // var o = select(this);
        arrow.select("image" + className("markarrow", true)).datum(diff).attr("x", diff.x).attr("y", diff.y).attr("height", diff.h).attr("width", diff.w).attr("xlink:href", diff.value < 0 ? downArrow : upArrow);
        arrow.select("text" + className("markarrow", true)).datum(diff).attr("x", diff.x + 25).attr("y", diff.y + 15).attr("height", diff.h).attr("width", diff.w).text(diff.text);
    };
...
function _domain$2() {
  ...
  yDomain = domainY$1(field.y, munged, level, aggregated, stacked, label);
...
function _range$2() {
  ...
  if (diffWithArrow) {
    var _d = 0;
    var _d2 = innerSize.width - _d;
    scale.x.range([_d + 100, _d2 - 200]);
  }
  scale.y.range([innerSize.height, 0]); //reverse
...
function translate$3(selection, innerSize, pos) {
    selection.attr("transform", "translate(" + [innerSize.width + pos[0], -20 + pos[1]] + ")");
}
function _unit$2() {
    var unit = this.unit();
    ...
    var unit_text = canvas.select(".unit-text");
    unit_text.text(unit ? unit['text'] ? unit['text'] : unit : "").style("font-family", "Noto Sans KR").style("font-size", "22px").style("font-weight", "600").style("fill", "#C6CBCE").attr("text-anchor", "end").call(translate$3, innerSize, unit ? unit['pos'] ? unit['pos'] : [0, 0] : [0, 0]);
}

function translate$4(selection, innerSize, pos) {
    selection.attr("transform", "translate(" + [pos[0] - 85, pos[1] - 20] + ")");
}
...
function _title$3() {
    ...
    var title_text = canvas.select(".title_text");
    title_text.text(title ? title['text'] ? title['text'] : title : "").style("font-family", "Noto Sans KR").style("font-size", "40px").style("font-weight", "bold").style("fill", "#FFFFFF").attr("text-anchor", "start").call(translate$4, innerSize, title ? title['pos'] ? title['pos'] : [0, 0] : [0, 0]);
}
...
var Line = function (_mix$with) {
  ...
  function Line() {
    ...
    _this.process('munge', _munge$2, { isPre: true }).process('unit', _unit$2).process('title', _title$3).process('brushZoom', _brushZoom, { isPre: true, allow: function allow() {
        return this.isBrushZoom();
      } }).process('domain', _domain$2, { isPre: true, allow: function allow() {
        return !this.isBrushZoom();
      } }).process('range', _range$2, { isPre: true, allow: function allow() {
        return !this.isBrushZoom();
      } }).process('axis', _axis$3, { allow: function allow() {
        return !this.isBrushZoom();
      } }).process('region', _region$2, { allow: function allow() {
        return !this.isBrushZoom();
      } }).process('facet', _facet$2, { allow: function allow() {
        return !this.isBrushZoom() && this.isFacet();
      } }).process('mark', _mark$2, { allow: function allow() {
        return !this.isBrushZoom() && !this.isFacet();
      } }).process('meanLine', _meanLine, { allow: function allow() {
        return !this.isBrushZoom();
        ...
        return !this.isBrushZoom();
      } }).process('brush', _brush, { allow: function allow() {
        return !this.isBrushZoom();
      } }).process('legend', _legend$3, { allow: function allow() {
        return !this.isBrushZoom();
      } });
    return _this;
  }
  ...
  createClass(Line, [{
    ...
    value: function renderCanvas() {
      return get(Line.prototype.__proto__ || Object.getPrototypeOf(Line.prototype), 'renderCanvas', this).call(this, this.point() ? this.size().range[0] * 2 : 0);
    }
    /**
...
function domainY$1(fieldY, munged) {
  ...
  var label = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false;

  var yDomain = fieldY.munged(munged).level(level).aggregated(aggregated).domain(0, stacked);
  if (label) {
    if (yDomain[0] === 0) yDomain[1] *= 1.1;else if (yDomain[1] === 0) yDomain[0] *= 1.1;else {
...
var Scatter = function (_mix$with) {
  ...
  createClass(Scatter, [{
    key: 'renderCanvas',
    value: function renderCanvas() {
      return get(Scatter.prototype.__proto__ || Object.getPrototypeOf(Scatter.prototype), 'renderCanvas', this).call(this, this.size().range[this.isSized() ? 1 : 0] * 1.25);
    }
  }, {
    key: 'isColor',
...
function _domain$7() {
  ...
  var xDomain = field.x.munged(munged).level(0).domain(this.sortByValue());
  var yDomain = field.y.munged(munged).level(1).domain(this.sortByValue());
  var colorDomain = field.color.munged(munged).level(1).domain();
...
var XYHeatmap = function (_mix$with) {
  ...
  return XYHeatmap;
}(mix(RectLinear).with(paddingMixin, sortMixin));
...
function _domain$11(keep) {
  if (this.isParcoords()) parCoords$1.call(this, keep);else matrix.call(this, keep);
}
...
function _range$10(keep) {
  if (this.isParcoords()) parCoords$2.call(this, keep);else matrix$1.call(this, keep);
}
...
var ParCoords = function (_mix$with) {
  ...
  function ParCoords() {
    ...
    _this.process('munge', _munge$10, { isPre: true }).process('domain', _domain$11, { isPre: true }).process('range', _range$10, { isPre: true }).process('axis', _axis$9).process('region', _region$8).process('mark', _mark$10, { allow: function allow() {
      ...
      } }).process('facet', _facet$6, { allow: function allow() {
        return !this.isParcoords();
      } }).process('legend', _legend$9).process('brush', _brush$2);
    return _this;
  }
...
function _munge$12() {
  ...
  if (this.sortByValue()) {
    pieGen.sortValues(this.sortByValue() === 'ascending' ? function (a, b) {
      return a - b;
    } : function (a, b) {
      return b - a;
    });
  } else {
    pieGen.sortValues(function (a, b) {
      return 0;
    });
  }
...
function _mark$12() {
  ...
  var size = this.size();
  var font = this.font();
  var textWithLabel = this.textWithLabel();
  ...
  var __labelInit = function __labelInit(selection) {
    selection.each(function (d) {
      d3.select(this).attr('x', d.dx).attr('dy', '.35em').attr('y', d.dy - font['font-size']).attr('text-anchor', 'middle').style('pointer-events', 'none').text(d.text);
  ...
  var __labelPercentInit = function __labelPercentInit(selection) {
    selection.each(function (d) {
      d3.select(this).attr('x', d.dx).attr('dy', '.35em').attr('y', d.dy + font['font-size']).attr('text-anchor', 'middle').style('pointer-events', 'none').text(d.percentage);
      that.styleFont(d3.select(this));
    });
  };

  var __labelPercent = function __labelPercent(selection) {
    selection.each(function (d) {
      d3.select(this).style('visibility', label ? 'visible' : 'hidden').text(d.percentage).transition(trans).attr('x', d.dx).attr('y', d.dy);
      that.styleFont(d3.select(this));
    });
  };

  var __labelTextInit = function __labelTextInit(selection) {
    selection.each(function (d) {
      d3.select(this).attr('x', d.dx).attr('dy', '.35em').attr('y', d.dy).attr('text-anchor', 'middle').style('pointer-events', 'none').text(d.labeltext);
      that.styleFont(d3.select(this));
    });
  };

  var __labelText = function __labelText(selection) {
    selection.each(function (d) {
      d3.select(this).style('visibility', label ? 'visible' : 'hidden').text(d.labeltext).transition(trans).attr('x', d.dx).attr('y', d.dy);
      that.styleFont(d3.select(this));
    });
  };
  ...
  var __appendNodes = function __appendNodes(selection) {
    ...
    if (showType === 'all' || showType === 'value') {
      nodeEnter.append('text').attr('class', that.nodeName(true) + " label").call(__labelInit);
    }
    if (showType === 'all' || showType === 'percent') {
      nodeEnter.append('text').attr('class', that.nodeName(true) + " labelpercent").call(__labelPercentInit);
    }
    node.call(__local);
    ...
    if (showType === 'all' || showType === 'value') {
      node.select('label').call(__label);
    }
    if (showType === 'all' || showType === 'percent') {
      node.select('labelpercent').call(__labelPercent);
    }
    that.__execs__.nodes = node;
  };
...
function translate$5(selection, innerSize) {
...
function _unit$4() {
    ...
    var unit_text = canvas.select(".unit-text");
    unit_text.text(unit).style("font-family", "Noto Sans KR").style("font-size", "22px").style("font-weight", "600").style("fill", "#C6CBCE").attr("text-anchor", "end").call(translate$5, innerSize);
}

function translate$6(selection, innerSize) {
...
function _title$5() {
    ...
    var title_text = canvas.select(".title_text");
    title_text.text(title).style("font-family", "Noto Sans KR").style("font-size", "40px").style("font-weight", "bold").style("fill", "#FFFFFF").attr("text-anchor", "start").call(translate$6, innerSize);
}
...
var Pie = function (_mix$with) {
  ...
  function Pie() {
    ...
    _this.process('munge', _munge$12, { isPre: true }).process('unit', _unit$4).process('title', _title$5).process('domain', _domain$13, { isPre: true }).process('mark', _mark$12).process('legend', _legend$11).process('tooltip', _tooltip$11);
    return _this;
  }
  ...
  return Pie;
}(mix(Core).with(paddingMixin, sortMixin));
...
function _domain$17(keep) {
  ...
  this.setCustomDomain('yBar', domainY(yBar, munged, 0, false, false, false, false, label));
  this.setCustomDomain('yLine', domainY$1(yLine, munged, 0, false, false, label));
...
function _facet$8() {
  ...
  var _smallLine = function _smallLine() {
    var smallLine = line$2().container(this).data(data).dimensions(dimensions).measures(lineMeasures).width(width).height(height).font(font).legend(false).tooltip(false).padding(padding).parent(parent).zeroOffset(true).color(color[1]).scaleBandMode(true);
...
var conditions$8 = ['normal', 'count', 'mixed'];
var _attrs$25 = {
  padding: 0.05,
  point: false,
  pointRatio: 3,
