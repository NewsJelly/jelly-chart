(function (global, factory) {
  ...
  function aggregate() {
    ...
    var sum = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : true;
  ...
  setMethodsFromAttrs(MeasureField, attrs$1);
        ...
        this.__execs__.autoResize = function () {
          if (allowResize && that.__execs__.canvas) {
            ...
            if (!lastWidth || lastWidth !== rect.width) {
              ...
              that.width(rect.width).transition({ duration: 0, delay: 0 }).render(true, that.autoResizeSkip()).transition(transition);
  ...
  function demuteRegions (exceptionFilter) {
    ...
    if (!arguments.length) {
      ...
      demute$1.call(this, regions);
      if (this.seriesName) demute$1.call(this, regions, true);
    } else {
  ...
  function muteRegions(exceptionFilter) {
    ...
    if (!arguments.length) {
      regions = this.filterRegions();
      mute$1.call(this, regions, true);
      if (this.seriesName) mute$1.call(this, regions, true, true);
    } else if (exceptionFilter === null) {
      regions = this.filterRegions();
      mute$1.call(this, regions, false);
      if (this.seriesName) mute$1.call(this, regions, false, true);
    } else {
      regions = this.filterRegions(conditionForMute(exceptionFilter), true);
      if (regions.size() > 0) {
        mute$1.call(this, regions, true);
  ...
  function render() {
    ...
    var dispatch = this.__execs__.selectDispatch;
    var node = this.nodes();
    ...
    if (node && node.size() > 0) {
      ...
      node.call(bindOn, dispatch).on('mouseenter.default.legend', function (d) {
  ...
  function appendClipPath(selection, innerSize) {
    ...
    var transition = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : null;
  ...
  function _arrow(selection, that, rowNum) {
    ...
    var rectW = labelHeight + labelPadding / 2;
    var arrowColor = d3.schemeCategory10[0],
        rectColor = '#A3A3A3';
    var area = selection.select(className('label-area', true));
    var curRowNum = 0;
    ...
    arrow = arrowEnter.merge(arrow).attr('transform', function (d, i) {
      if (isHorizontal) return 'translate(' + [width - rectW, i * (labelHeight + labelPadding / 2)] + ')';else return 'translate(' + [0, height + labelPadding * 1.25 + i * (labelHeight + labelPadding)] + ')';
    });
  ...
  function _render(selection) {
    ...
    if (area.empty()) {
      area = selection.append('g').attr('class', className('label-area-parent')).append('g').attr('class', className('label-area'));
    }

    var labelHeight = this.labelHeight();
    var labelHeightHalf = labelHeight / 2;
    var scale = this.scale();
    var label = area.selectAll(className('label', true)).data(scale.domain().filter(function (d) {
    ...
    var dispatch = this.__execs__.dispatch;
  ...
  function demute$2(exceptionFilter) {
  ...
  function mute$2(exceptionFilter) {
  ...
  function renderLegend() {
    ...
    var offsetThickness = legendToggle.font ? legendToggle.font['font-size'] : 10;
  ...
  var _attrs$1 = {
    ...
    orient: orients$1[0],
    gradientWidth: 150,
    gradientHeight: defaultFont$1['font-size'],
    scale: null,
    showTooltip: true,
    title: null,
    width: 0,
  ...
  var Spectrum = function Spectrum() {
    ...
    this.__attrs__ = JSON.parse(JSON.stringify(_attrs$1));
  ...
  function _render$1(selection) {
    ...
    var area = selection.select(className('gradient-area', true));
    if (area.empty()) {
      ...
      area.append('path').attr('class', className('arrow')).attr('fill', '#777').attr('d', triangleSymbol).style('visibility', 'hidden');
    ...
    if (safari()) {
      ...
      var rect = area.selectAll(className('gradient-rect', true)).data([scale]);
    ...
    var label = area.selectAll(className('label', true)).data(scale.domain());
    ...
    var title = area.selectAll(className('title', true)).data([titleText]);
    title.exit().remove();
  ...
  Spectrum.prototype.render = render$2;
  Spectrum.prototype.show = show;
  Spectrum.prototype.update = update$1;

  setMethodsFromAttrs(Spectrum, _attrs$1);
  ...
  var Tooltip = function Tooltip() {
    ...
    this.__attrs__ = JSON.parse(JSON.stringify(_attrs$2));
  ...
  Tooltip.prototype.render = render$3;
  ...
  setMethodsFromAttrs(Tooltip, _attrs$2);
  ...
  function fillUndefined(range, colors) {
    ...
    return range.filter(function (c) {
      return c !== undefined;
    });
  }
  /**
  ...
  Core.prototype.font = setMethodFromDefaultObj('font', defaultFont$3);
  ...
  function _generator() {
    ...
    if (orient === orients$2[0]) axis = d3.axisTop();else if (orient === orients$2[1]) axis = d3.axisRight();else if (orient === orients$2[3]) axis = d3.axisLeft();else axis = d3.axisBottom();
  ...
  function facet(orient) {
    ...
    if (orients$2.includes(orient)) this.__attrs__.facet = { orient: orient };else this.__attrs__.facet = { orient: orients$2[0] };
    return this;
  }
  ...
  var orients$3 = ['vertical', 'horizontal'];
  /**
  ...
  function facet$1() {
    ...
    if (!arguments.length) return this.__attrs__.facet;
    if (!facet) this.__attrs__.facet = false;else if (orients$3.includes(facet)) this.__attrs__.facet = { orient: facet };else this.__attrs__.facet = { orient: orients$3[0] };
    return this;
  }
  ...
  var paddingMixin = function paddingMixin(Base) {
    ...
    */
    ...
    setMethodsFromAttrs(PaddingMixin, _attrs$7);
    return PaddingMixin;
  };
  ...
  function line(selection, innerSize) {
    var isVertical = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
  ...
  function _annotation() {
    ...
    annoEnter.append('line').style('stroke', annotation.color).style('shape-rendering', 'crispEdges').call(line, innerSize, isVertical);
    ...
    anno.select('line').transition(trans).style('stroke', annotation.color).call(line, innerSize, isVertical);
  }
  ...
  var Bar = function (_mix$with) {
    ...
    function Bar() {
      ...
      _this.process('munge', _munge, { isPre: true }).process('domain', _domain, { isPre: true }).process('range', _range, { isPre: true }).process('axis', _axis$1).process('region', _region).process('facet', _facet, { allow: function allow() {
          return this.isFacet();
    ...
    createClass(Bar, [{
      ...
      value: function isVertical() {
        return this.orient() === orients$4[0];
      }
    }, {
  ...
  function brushGen(brushGen) {
    ...
    var dispatch = this.__execs__.brushDispatch;
  ...
  function zoom(zoom) {
    ...
    if (typeof zoom === 'boolean') {
      if (zoom) this.__attrs__.zoom = 'normal';else this.__attrs__.zoom = false;
    } else if (typeof zoom === 'string') {
      if (types.findIndex(function (d) {
        return d.localeCompare(zoom) === 0;
      }) >= 0) {
  ...
  function zoomGen(_val) {
    ...
    var that = this;
    var dispatch = that.__execs__.zoomDispatch;
  ...
  var zoomMixin = function zoomMixin(Base) {
    ...
    setMethodsFromAttrs(ZoomMixin, _attrs$14);
    return ZoomMixin;
  };
  ...
  function _brushZoom() {
    ...
    region.each(function (d, i) {
      var l = line$1().container(this).data(that.data()).dimensions(that.dimensions()).measures(that.measures()).width(innerSize.width).height(d.h).legend(false).zeroMargin(true).parent(that);
      settings.forEach(function (d) {
    ...
    lines[1].on('brushed.line brushEnd.line', function () {
      ...
      var range = d3.event.selection || brushScale.range();
      var domain = range.map(brushScale.invert);
      range = domain.map(zoomScale);
      lines[0].zoomTransform(zoomGroup, d3.zoomIdentity.scale((zoomScale.range()[1] - zoomScale.range()[0]) / (range[1] - range[0])).translate(-range[0], 0));
    });
  }
  ...
  function _mark$1() {
    ...
    var __seriesInit = function __seriesInit(selection) {
      var area = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    ...
    var __appendSeries = function __appendSeries(selection) {
      ...
      var ___append = function ___append(selection, area) {
        var path = selection.selectAll('path' + className(area ? 'area' : 'line', true)).data(function (d) {
          return [d];
        });
        path.exit().remove();
        path.enter().append('path').attr('class', className(area ? 'area' : 'line')).call(__seriesInit, area).merge(path, area).call(__series, area).style('pointer-events', 'none');
      };
      if (isArea) {
  ...
  function _facet$1() {
    ...
    var _smallLine = function _smallLine(d) {
      var smallLine = line$1().container(this).data(d).dimensions(dimensions).measures(measures).width(width).height(height).legend(false).tooltip(false).parent(parent).zeroMargin(true).aggregated(true).color(scale.color(d.data.key));
      settings.forEach(function (d) {
  ...
  var MultiTooltip = function MultiTooltip() {
    ...
    this.__attrs__ = JSON.parse(JSON.stringify(_attrs$16));
  ...
  MultiTooltip.prototype.render = render$5;
  MultiTooltip.prototype.tick = tick;

  setMethodsFromAttrs(MultiTooltip, _attrs$16);
  ...
  function _multi() {
    ...
    var dispatch = this.__execs__.multiTooltipDispatch;
  ...
  function normal() {
    ...
    this.zoomGen(zoomGen).zoomed(function () {
      _mark$1.call(_this, true); //reset marks
      ...
      _tooltip$2.call(_this); //reset tooltips
    });
  }
  ...
  var size$1 = { range: [2, 2], scale: 'linear', reverse: false };
  ...
  var Line = function (_mix$with) {
    ...
    function Line() {
      ...
      _this.process('munge', _munge$1, { isPre: true }).process('brushZoom', _brushZoom, { isPre: true, allow: function allow() {
          return this.isBrushZoom();
        } }).process('domain', _domain$1, { isPre: true, allow: function allow() {
          return !this.isBrushZoom();
        } }).process('range', _range$1, { isPre: true, allow: function allow() {
          return !this.isBrushZoom();
        } }).process('axis', _axis$2, { allow: function allow() {
          return !this.isBrushZoom();
        } }).process('region', _region$1, { allow: function allow() {
          return !this.isBrushZoom();
        } }).process('facet', _facet$1, { allow: function allow() {
          return !this.isBrushZoom() && this.isFacet();
        } }).process('mark', _mark$1, { allow: function allow() {
          return !this.isBrushZoom() && !this.isFacet();
        } }).process('meanLine', _meanLine, { allow: function allow() {
          return !this.isBrushZoom();
        } }).process('fitLine', _fitLine, { allow: function allow() {
          return !this.isBrushZoom();
        } }).process('tooltip', _tooltip$2, { allow: function allow() {
          return !this.isBrushZoom();
        } }).process('zoom', _zoom, { allow: function allow() {
          return !this.isBrushZoom();
        } }).process('brush', _brush, { allow: function allow() {
          return !this.isBrushZoom();
        } }).process('legend', _legend$2, { allow: function allow() {
          return !this.isBrushZoom();
        } });
      return _this;
    }
  ...
  function normal$1() {
    ...
    this.zoomGen(zoomGen).zoomed(function () {
      _mark$2.call(_this, true); //re-render mark
      ...
      _tooltip$3.call(_this); //re-render tooltip
    }, true);
  }

  function _zoom$1() {
    if (!this.zoom()) return;
    if (this.zoom() === 'normal') normal$1.call(this);else if (this.zoom() === 'brush') _brushZoom$1.call(this);
  }

  var size$2 = { range: [3, 12], scale: 'linear', reverse: false };
  ...
  var Scatter = function (_mix$with) {
    ...
    function Scatter() {
      ...
      _this.process('munge', _munge$2, { isPre: true }).process('domain', _domain$3, { isPre: true }).process('range', _range$2, { isPre: true }).process('axis', _axis$3).process('fitLine', _fitLine$1).process('region', _region$2).process('facet', _facet$2, { allow: function allow() {
          return this.isFacet();
        } }).process('mark', _mark$2, { allow: function allow() {
          return !this.isFacet();
        } }).process('legend', _legend$3).process('tooltip', _tooltip$3).process('zoom', _zoom$1);
      return _this;
    }
  ...
  var XYHeatmap = function (_mix$with) {
    ...
    function XYHeatmap() {
      ...
      _this.process('munge', _munge$3, { isPre: true }).process('domain', _domain$4, { isPre: true }).process('range', _range$3, { isPre: true }).process('region', _region$3).process('axis', _axis$4).process('mark', _mark$3).process('spectrum', _legend$4).process('tooltip', _tooltip$4);
      return _this;
    }
  ...
  var Treemap = function (_mix$with) {
    ...
    function Treemap() {
      ...
      _this.process('munge', _munge$4, { isPre: true }).process('domain', _domain$5, { isPre: true }).process('range', _range$4, { isPre: true }).process('mark', _mark$4).process('spectrum', _legend$5).process('tooltip', _tooltip$5);
      return _this;
    }
  ...
  function _domain$6(keep) {
    if (this.isParcoords()) parCoords.call(this, keep);else matrix.call(this, keep);
  }

  function parCoords$1() {
    var scale = this.scale();
    var innerSize = this.innerSize();
    var measures = this.measures();
    scale.x.rangeRound([0, innerSize.width]);
  ...
  function _range$5(keep) {
    if (this.isParcoords()) parCoords$1.call(this, keep);else matrix$1.call(this, keep);
  }
  ...
  var size$3 = { range: [1, 1], scale: 'linear', reverse: false };
  ...
  var ParCoords = function (_mix$with) {
    ...
    function ParCoords() {
      ...
      _this.brush(true);
      _this.process('munge', _munge$5, { isPre: true }).process('domain', _domain$6, { isPre: true }).process('range', _range$5, { isPre: true }).process('axis', _axis$5).process('region', _region$4).process('mark', _mark$5, { allow: function allow() {
          return this.isParcoords();
        } }).process('facet', _facet$3, { allow: function allow() {
          return !this.isParcoords();
        } }).process('legend', _legend$6).process('brush', _brush$1);
      return _this;
    }
  ...
  var parCoords$2 = genFunc(ParCoords);
  ...
  var size$4 = { range: [0, 150], scale: 'linear', reverse: false };
  ...
  var Pie = function (_mix$with) {
    ...
    function Pie() {
      ...
      _this.process('munge', _munge$6, { isPre: true }).process('domain', _domain$7, { isPre: true }).process('mark', _mark$6).process('legend', _legend$7).process('tooltip', _tooltip$6);
      return _this;
    }
  ...
  var size$5 = { range: [0.5, 4], scale: 'linear', reverse: false };
  ...
  var MarkerMap = function (_Default) {
    ...
    function MarkerMap() {
      ...
      _this.process('munge', _munge$7, { isPre: true }).process('domain', _domain$8, { isPre: true }).process('map', _map).process('mark', _mark$7);
      return _this;
    }
  ...
  function _facet$4() {
    ...
    var _smallLine = function _smallLine() {
      var smallLine = line$1().container(this).data(data).dimensions(dimensions).measures(lineMeasures).width(width).height(height).legend(false).tooltip(false).padding(padding).parent(parent).zeroOffset(true).color(color[1]).scaleBandMode(true);
  ...
  var size$6 = { range: [2, 2], scale: 'linear', reverse: false };
  ...
  var Combo = function (_mix$with) {
    ...
    function Combo() {
      ...
      _this.process('munge', _munge$8, { isPre: true }).process('domain', _domain$9, { isPre: true }).process('region', _region$5).process('facet', _facet$4).process('axis', _axis$6).process('legend', _legend$8).process('tooltip', _tooltip$7);
      return _this;
    }
    /**
     * @override
